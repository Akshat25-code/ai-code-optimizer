...
class LanguageValidationError(ValueError):
    ...
    def _generate_error_message(self, language: str) -> str:
        ...
        if language in NON_PROGRAMMING_LANGUAGES:
            ...
            return (
                f"âŒ Unsupported Language: '{language}' is not currently supported.\n"
                f"âœ… Supported languages: {', '.join(SUPPORTED_LANGUAGES.keys())}\n"
                f"ðŸ”¥ Popular choices: {', '.join(popular_languages)}"
            )
...
if __name__ == "__main__":
    ...
    for test_lang in test_cases:
        try:
            ...
            print(f"âœ… {test_lang}: Valid ({info.description})")
        except LanguageValidationError as e:
            print(f"âŒ {test_lang}: {e}")
