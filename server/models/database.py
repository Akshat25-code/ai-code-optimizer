...
from core.database import get_database, serialize_doc, get_object_id
...
class MongoOAuthProvider:
    ...
    async def create_oauth_link(user_id: str, provider: str, provider_id: str, access_token: str) -> Dict[str, Any]:
        """
        Link OAuth provider to user. Stores access_token encrypted at rest.

        If a link for the same (user_id, provider) already exists, the previous
        link is preserved as an audit record with ``previous_provider_id`` and
        ``rotated_at`` fields before being superseded. This prevents silent loss
        of identity history when a user re-links the same provider.
        """
        db = get_database()

        # Encrypt before persistence.
        encrypted = encrypt_str(access_token)
        if encrypted is None:
            raise ValueError("Failed to encrypt OAuth access token")

        existing = await db.oauth_providers.find_one({"user_id": user_id, "provider": provider})
        if existing:
            # Rotate: keep an audit trail, do not silently overwrite.
            await db.oauth_oauth_audit.insert_one({
                "user_id": user_id,
                "provider": provider,
                "previous_provider_id": existing.get("provider_id"),
                "previous_access_token": existing.get("access_token"),
        ...
        oauth_doc = {
            ...
            "previous_provider_id": existing.get("provider_id") if existing else None,
            "created_at": datetime.now(timezone.utc),
            "updated_at": datetime.now(timezone.utc),
        }
    ...
    async def find_token_by_user(user_id: str, provider: str) -> Optional[str]:
        """Find OAuth access token for a user and provider (decrypted)."""
        db = get_database()
        doc = await db.oauth_providers.find_one({"user_id": user_id, "provider": provider})
        if not doc:
            return None
        return decrypt_str(doc.get("access_token"))

class MongoPhoneOTP:
    """One-time password (OTP) storage for phone login verification.
    OTPs are stored as a hash only. The plaintext code is never persisted.
    """

    @staticmethod
    async def create_otp(phone: str, code_hash: str, ttl_minutes: int = 5) -> Dict[str, Any]:
        db = get_database()
...
# Helper functions for authentication (used by api/auth_routes.py)
...
async def create_user_session(user_id: str, access_token: str, refresh_token: str, **kwargs) -> Dict[str, Any]:
    """Create a new user session with hashed tokens."""
