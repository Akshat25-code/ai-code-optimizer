...
- **Time Complexity**: [Old] â†’ [New]
- **Space Complexity**: [Old] â†’ [New]
...
ðŸ” **Bug Analysis**: [Check for errors and issues]
âš¡ **Optimization Opportunities**: [Performance improvements]
ðŸ“š **Code Explanation**: [What the code does and how]
ðŸ—ï¸ **Structural Improvements**: [Better organization and design]
ðŸ“– **Documentation**: [How to improve code documentation]
ðŸŽ¯ **Best Practices**: [Apply {language} conventions and patterns]
...
async def ask_openai(prompt: str, *, api_key_override: str | None = None) -> str:
...
async def ask_claude(prompt: str, *, api_key_override: str | None = None) -> str:
...
async def ask_gemini(prompt: str, *, api_key_override: str | None = None) -> str:
...
async def ask_deepseek(prompt: str, *, api_key_override: str | None = None) -> str:
    key = api_key_override or settings.deepseek_api_key
    if not key:
...
async def ask_grok(prompt: str, *, api_key_override: str | None = None) -> str:
    key = api_key_override or settings.grok_api_key or settings.groq_api_key
    if not key:
        raise ProviderConfigError("Grok key missing (set GROK_API_KEY or GROQ_API_KEY)")

    using_groq_fallback = bool(not api_key_override and settings.groq_api_key and not settings.grok_api_key)
    headers = {"Authorization": f"Bearer {key}"}
...
async def ask_ai(
    ...
    # Map provider names to (ask_function, key_dict_field)
    _provider_map = {
        "openai": ("openai", ask_openai),
        "claude": ("anthropic", ask_claude),
        "gemini": ("gemini", ask_gemini),
        "deepseek": ("deepseek", ask_deepseek),
        "grok": ("grok", ask_grok),
    }

    def _get_key(prov: str) -> str | None:
        if not api_keys:
            return None
        mapping = {"openai": "openai", "claude": "anthropic", "gemini": "gemini",
                    "deepseek": "deepseek", "grok": "grok"}
        return api_keys.get(mapping.get(prov, prov))

    async def run_and_track(p: str, ask_fn, prov_key: str):
        override = _get_key(prov_key)
        res = await ask_fn(prompt, api_key_override=override)
        return p, res, prompt_tokens, count_tokens(res)

    try:
        if selected == "openai":
            return await run_and_track("openai", ask_openai, "openai")
        elif selected == "claude":
            return await run_and_track("claude", ask_claude, "claude")
        elif selected == "gemini":
            return await run_and_track("gemini", ask_gemini, "gemini")
        elif selected == "deepseek":
            return await run_and_track("deepseek", ask_deepseek, "deepseek")
        elif selected == "grok":
            return await run_and_track("grok", ask_grok, "grok")
        else:
        ...
        for fallback in fallback_providers:
            try:
                if fallback == "openai":
                    return await run_and_track("openai(fallback)", ask_openai, "openai")
                elif fallback == "claude":
                    return await run_and_track("claude(fallback)", ask_claude, "claude")
                elif fallback == "deepseek":
                    return await run_and_track("deepseek(fallback)", ask_deepseek, "deepseek")
                elif fallback == "grok":
                    return await run_and_track("grok(fallback)", ask_grok, "grok")
                else:
                    return await run_and_track("gemini(fallback)", ask_gemini, "gemini")
            except Exception:
                continue
        raise e
