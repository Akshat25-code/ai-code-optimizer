...
from mongodb_auth_models import MongoUser, MongoUserSession, MongoOAuthProvider, create_user_session
from jwt_utils import PasswordManager, SessionManager, create_tokens_for_user
...
def _popup_html(tokens: Dict[str, Any]) -> str:
        ...
        # Note: In dev we post to '*'. Frontend should validate origin.
    ...
    (function() {{
        try {{
            ...
            if (window.opener && !window.opener.closed) {{
                window.opener.postMessage({{ source: 'oauth', payload: payload }}, '*');
            }}
...
async def google_start(request: Request):
    ...
    return_to = request.query_params.get("redirect_uri")  # optional frontend URL
