...
const focusOptions = [
  { key: 'time_complexity', label: 'Time Complexity', icon: 'â±ï¸' },
  { key: 'space_complexity', label: 'Space Complexity', icon: 'ðŸ“¦' },
  { key: 'code_length', label: 'Code Length', icon: 'ðŸ“' },
  { key: 'readability', label: 'Readability', icon: 'ðŸ‘ï¸' },
  { key: 'performance', label: 'Performance', icon: 'ðŸš€' },
  { key: 'memory', label: 'Memory', icon: 'ðŸ’¾' },
];

const OptimizationPage = () => {
  const optimizer = useOptimizer({ defaultTask: 'optimization', enableFileManager: true });
                ...
                <button
                  onClick={() => {
                    if (optimizer.backendHealthy === false) {
                      optimizer.setToast({ type: 'error', message: 'âš ï¸ Backend server is not running.' });
            ...
            >
              ...
              <div className="glass-frame-header bg-[var(--surface-2)]">
                <span className="text-sm font-bold text-[var(--accent-cyan)] flex items-center gap-2">
                  <span className="text-xl filter drop-shadow-[0_0_10px_var(--accent-cyan)]">ðŸ“Š</span> Performance Matrix
                </span>
              </div>
              <div className="p-5 relative z-10">
                <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
                  {[
                    {
                      icon: 'â±ï¸', title: 'Time Complexity',
                      val1: optimizer.quickMetrics?.timeComplexityBefore || 'O(n)',
                      val2: optimizer.quickMetrics?.timeComplexityAfter || 'O(1)',
                      extra: `${optimizer.quickMetrics?.originalLines ?? 0} â†’ ${optimizer.quickMetrics?.optimizedLines ?? 0} Lines`
                    },
                    {
                      icon: 'ðŸ“¦', title: 'Space Complexity',
                      val1: optimizer.quickMetrics?.spaceComplexityBefore || 'O(n)',
                      val2: optimizer.quickMetrics?.spaceComplexityAfter || 'O(1)',
                      extra: `${optimizer.quickMetrics?.originalChars ?? 0} â†’ ${optimizer.quickMetrics?.optimizedChars ?? 0} Chars`
                    },
                    {
                      icon: 'ðŸ“', title: 'Lines of Code',
                    ...
                    {
                      icon: 'ðŸ—œï¸', title: 'Code Size (Bytes)',
